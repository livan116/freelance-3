...
if (isset($_POST['submit'])) {
    ...
    $guardianSignature = htmlspecialchars($_POST['guardianSignature'] ?? '');
    ...
    if (!empty($signature)) {
        ...
        file_put_contents('signatures/signature_' . time() . '.png', $signatureData);
    }
    
    if (!empty($guardianSignature)) {
        $guardianSignatureData = base64_decode(preg_replace('#^data:image/\w+;base64,#i', '', $guardianSignature));
        file_put_contents('signatures/guardian_signature_' . time() . '.png', $guardianSignatureData);
    }
...
<body class="bg-white font1 text-gray-800">
   ...
   <nav class="Navbar shadow-md z-50 px-3 transition-all duration-300" id="navbar">
    ...
    canvas.style.border = '1px solid #000'; // Add border for visibility
    ...
    return { canvas, ctx, getSignatureDataURL };
}
    ...
    form.addEventListener('submit', function (e) {
        // Capture the signature based on the selected section
        if (under18Yes.checked) {
            const guardianSignatureDataURL = guardianSignaturePad.getSignatureDataURL();
            const guardianSignatureInput = document.createElement('input');
            guardianSignatureInput.type = 'hidden';
            guardianSignatureInput.name = 'guardianSignature';
            guardianSignatureInput.value = guardianSignatureDataURL;
            form.appendChild(guardianSignatureInput);
        } else {
            const adultSignatureDataURL = adultSignaturePad.getSignatureDataURL();
            const adultSignatureInput = document.createElement('input');
            adultSignatureInput.type = 'hidden';
            adultSignatureInput.name = 'signature';
            adultSignatureInput.value = adultSignatureDataURL;
            form.appendChild(adultSignatureInput);
        }
    });
});
